...
handlers._users.put = function(data,callback){
  ...
  if(phone){
    ...
    if(firstName || lastName || password){
      // Get the token from the headers
      var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
      // Verify if the token is token is valid
      handlers._tokens.verifyToken(token,phone,function(tokenIsValid){
        if(tokenIsValid){
          // Lookup the user
        ...
        _data.read('users',phone,function(err,userData){
          if(!err && data){
            _data.delete('users',phone,function(err){
              if(!err){
                // Delete each of the checks associated with that users.
                    ...
                    _data.delete('checks',checkId,function(err){
                      ...
                      if(checksDeleted == checksToDelete){
                        if(!deletionErrors){
                          ...
                          callback(500, {'Errors' : 'Errors encountered while deleting the checks one or more checks could not be deleted successfully'});
                        }
                      }
                    });
                  });
    ...
    _data.read('users',phone,function(err,userData){
      if(!err && userData){
        ...
        if(hashedPassword == userData.hashedPassword){
          ...
          callback(400, {'Error' : 'Password did not match with the specified user'});
        }
      } else {
        callback(400, {'Error': 'Could not find the specified user'});
      }
    ...
    _data.read('tokens',id, function(err,tokenData){
      if(!err && tokenData){
        ...
        if(tokenData.expires > Date.now()){
          ...
          callback(400, {'Error' : 'The token is already expired cannot be extended'})
        }
      }
    });
  } else {
      ...
      if(!err && tokenData){
        ...
        _data.read('users',userPhone,function(err,userData){
          if(!err && userData){
            ...
            // Verify that the user has less than the number of max check per users
            ...
            if(tokenIsValid){
              ...
              _data.update('checks',id,checkData,function(err){
                if(!err){
                  ...
                  callback(500, {'Error' : 'Could not update the Checks Resource'});
                }
              });
            } else {
              callback(403);
              ...
              if(!err){
                ...
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    callback(400, {'Error':'Could not find the specified user so could not delete the check on the user object'});
                  }
                });
              } else {
