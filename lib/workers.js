      ...
      checks.forEach(function(check){
        ...
        _data.read('checks',check,function(err,originalCheckData){
          if(!err && originalCheckData){
            // Pass it to the check validator, and let that function handle the check validation.
...
// Process the checkOutcome and update the checkdata and trigger the alert.
...
workers.alertUserToStatusChange = function(newCheckData){
  ...
  helpers.sendTwilioSms(newCheckData.userPhone,msg,function(err){
    if(!err){
      ...
      debug("Error: Could not send sms to user for their change in status check");
    }
  });
};
